...
| 23 — autonomous showcase | PROOF PENDING; GITHUB MODELS REMOVED | [run 29684882715](https://github.com/johannhartmann/xlsliberator-swe/actions/runs/29684882715) passed exact image builds, hostile-sandbox hydration, trusted-service startup, and API trigger, then GitHub Models returned HTTP 429. GitHub Models and the workflow-token inference permission have since been removed. The manually dispatched workflow now requires explicit paid-usage authorization and supports direct OpenAI, Anthropic, Google, or Fireworks credentials; no candidate, independent `APPROVE`, or human-use proof exists yet |

## Next action

Configure an authorized direct-provider credential and manually dispatch Prompt
23 with that provider, explicitly authorizing paid usage for the run. Completion
