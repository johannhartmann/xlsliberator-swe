...
def _status(
    ...
    return next(
        ...
        if result.evaluator is evaluator
    )
...
def test_all_fourteen_evaluators_pass_or_truthfully_skip() -> None:
    ...
    assert (
        _status(report, EvaluatorName.GENERIC_REPAIR_REUSE)
        is EvaluationStatus.SKIPPED
    )
    assert all(
...
def test_required_corpus_security_hidden_and_reviewer_gates_block_release() -> None:
    ...
    assert (
        _status(report, EvaluatorName.REVIEWER_AGREEMENT)
...
def test_fake_success_test_weakening_and_weak_source_tests_are_detected() -> None:
    ...
    assert (
        _status(report, EvaluatorName.NO_TEST_WEAKENING)
        is EvaluationStatus.FAILED
    )
    assert (
        _status(report, EvaluatorName.SOURCE_DERIVED_TEST_QUALITY)
        is EvaluationStatus.FAILED
    )
...
def test_delegation_skills_mutations_dependency_and_repair_reuse_are_scored() -> None:
    ...
    assert (
        _status(report, EvaluatorName.SPECIALIST_DELEGATION)
        is EvaluationStatus.FAILED
    )
    assert _status(report, EvaluatorName.SKILL_SELECTION) is EvaluationStatus.FAILED
    assert (
        _status(report, EvaluatorName.MUTATION_KILL_RATE)
        is EvaluationStatus.FAILED
    )
    assert (
        _status(report, EvaluatorName.PROPRIETARY_DEPENDENCY_REMOVAL)
        is EvaluationStatus.FAILED
    )
    assert (
        _status(report, EvaluatorName.GENERIC_REPAIR_REUSE)
...
def test_unavailable_not_run_skipped_and_failed_remain_distinct() -> None:
    ...
    assert (
        _status(report, EvaluatorName.HIDDEN_ACCEPTANCE)
        is EvaluationStatus.UNAVAILABLE
    )
    assert (
        _status(report, EvaluatorName.MUTATION_KILL_RATE)
        is EvaluationStatus.NOT_RUN
    )
...
def test_manual_intervention_cost_and_latency_are_release_evidence() -> None:
    ...
    assert (
        _status(report, EvaluatorName.MANUAL_INTERVENTION_RATE)
        is EvaluationStatus.FAILED
    )
...
def test_langsmith_trace_metadata_records_required_dimensions_without_hidden_cases() -> None:
    ...
    assert metadata["xlsliberator_evidence_paths"]["hidden"] == (
